...
## Расширение браузера для заданий МЭШ

В каталоге `extension/` находится кросс-браузерное расширение **«МЭШ: все задания»**. Оно работает локально: сканирует открытую страницу МЭШ, собирает найденные карточки заданий и показывает их на отдельной странице расширения. Пароли, API-токены и содержимое заданий на внешний сервер не отправляются.

Расширение сейчас использует DOM-сканирование, поэтому оно не зависит напрямую от внутренних API МЭШ. Для динамических страниц предусмотрен запуск сканирования после полной загрузки страницы и через кнопку «Собрать задания».

### Сборка

Для сборки нужен только Python 3.9 или новее. Node.js и npm не требуются.
...
Скрипт создаёт следующие результаты:

```text
dist/chrome/                  # распакованная сборка Chromium
dist/firefox/                 # распакованная сборка Firefox/LibreWolf
...
Открой `about:debugging#/runtime/this-firefox`, нажми **«Загрузить временное дополнение»** и выбери файл `dist/firefox/manifest.json`. Для временной установки нужно выбирать именно `manifest.json`, а не ZIP-архив. После перезапуска LibreWolf временное дополнение обычно потребуется загрузить снова.

Для Chromium открой `chrome://extensions`, включи **режим разработчика**, нажми **«Загрузить распакованное расширение»** и выбери каталог `dist/chrome/`.

После установки открой страницу домашних заданий на `school.mos.ru` или `dnevnik.mos.ru`. Нажми кнопку расширения или плавающую кнопку **«Собрать задания»**. Результат откроется в отдельной вкладке.

### Ограничения текущего MVP

Сканер специально не пытается читать cross-origin iframe и не воспроизводит тестовый плеер МЭШ. Кнопки материалов ведут в оригинальный интерфейс, поэтому авторизация и выполнение теста остаются под контролем официальной страницы. Селекторы DOM сделаны эвристическими; если верстка МЭШ изменится, потребуется обновить `extension/src/content.js`.
