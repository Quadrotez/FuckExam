...
> В текущем MVP «выход из контейнера» означает потерю фокуса окна виртуальной машины на хостовой ОС. При обнаружении потери фокуса приложение выполняет небольшое движение мыши влево-вправо. По умолчанию включён `dry-run`, поэтому реальное движение курсора отключено до явного флага `--allow-input`.
...
| Input action | `pynput` или dry-run | Движение физического курсора хоста |
...
- для реального движения мыши: пакет `pynput` и разрешение ОС на управление вводом.
...
В `dry-run` приложение только пишет в лог, что движение было бы выполнено. Для разрешения реального ввода:
...
--allow-input               включить реальное движение курсора
...
├── actions.py        dry-run и физическое движение мыши
...
- **Dry-run first:** реальное управление курсором всегда требует явного `--allow-input`.
...
### Почему начат с VBoxManage

SDK VirtualBox требует установки соответствующих Python bindings/COM/XPCOM компонентов и отличается по платформам. `VBoxManage` уже поставляется с VirtualBox, стабилен для команд управления и удобен как boundary для первой версии. В следующем этапе можно добавить `pyvbox`/COM adapter с тем же контрактом `VirtualBoxClient` без переписывания monitor/action слоя.
...
- `IConsole::mouse` только если продуктовая политика потребует движения **внутри гостя**, а не физического курсора хоста;
...
Приложение управляет мышью на уровне хоста, поэтому его нельзя запускать с `--allow-input` в непроверенном окружении или поверх приложений, где неожиданный ввод опасен. Для демонстрации на конференции рекомендуется отдельная гостевая VM, `dry-run` на репетициях, минимальная амплитуда движения и ручная остановка через `Ctrl+C`.
